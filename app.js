(() => {
  ...
  const psi=document.getElementById('psi'),fio2=document.getElementById('fio2'),flow=document.getElementById('flow'),transportHours=document.getElementById('transportHours'),transportMinutes=document.getElementById('transportMinutes');
  ...
  function setTransportAssessment(durationMinutes){
    const hoursText=transportHours.value.trim(),minutesText=transportMinutes.value.trim();
    const hours=hoursText===''?0:Number(hoursText),minutes=minutesText===''?0:Number(minutesText);
    const hasEntry=hoursText!==''||minutesText!=='';
    ...
    const valid=!hasEntry||(validHours&&validMinutes&&t>0);
    document.getElementById('transportTimeError').hidden=valid;
    if(!hasEntry||!valid||durationMinutes===null||!Number.isFinite(durationMinutes)){transportAssessment.hidden=true;return;}
  ...
  [psi,fio2,flow,transportHours,transportMinutes].forEach(input=>input.addEventListener('input',calculate));
  document.getElementById('resetButton').addEventListener('click',()=>{psi.value='';fio2.value='';flow.value='';transportHours.value='';transportMinutes.value='';selectedTank='d';lastDuration=null;document.querySelectorAll('.source-option').forEach((item,i)=>{item.classList.toggle('selected',i===0);item.setAttribute('aria-checked',String(i===0));});document.querySelectorAll('.error').forEach(e=>e.hidden=true);setState('neutral','Awaiting Settings','Enter pressure, FiO₂, and flow');transportAssessment.hidden=true;psi.focus();});
